  ...
  for (genvar i = 0; i < NumWideBanks; i++) begin : gen_wide_banks
    ...
    for (genvar j = 0; j < NWDivisor; j++) begin : gen_narrow_banks
      ...
      for (genvar k = 0; k < NumPhysicalBanks; k++) begin : assign_pwr_control_sigs
        ...
        assign impl_narrow[k] = impl_i[(NUM_PWR_DOMAINS * k * NWDivisor)/(NumPhysicalBanks*NumWideBanks * NumPhysicalBanks * NWDivisor) + (NUM_PWR_DOMAINS * (i * NumPhysicalBanks * NWDivisor))/(NumWideBanks * NumPhysicalBanks * NWDivisor) + k];
      end
