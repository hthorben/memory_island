...
module axi_memory_island_tb #(
  ...
  logic [TotalReq-1:0] mismatch_mixed;
  ...
  int unsigned errors;
  int unsigned not_blocked;
  int unsigned blocked;

  // Function with fixed width vector
  function automatic int count_xs(input logic [TotalReq/2 - 1:0] vec);
    int count = 0;
  ...
  initial begin
    ...
    do begin
      #TestTime;
      errors += $countones(mismatch[(TotalReq/2) - 1 : 0]);
      blocked += count_xs(mismatch[TotalReq-1:TotalReq/2]);
      not_blocked += $countones(mismatch[TotalReq-1:TotalReq/2]);
      if (end_of_sim == '1) begin
        $display("Counted %d errors.", errors);
        $display("Power Gated accesses: %d Power Gating Errors: %d.", blocked, not_blocked);
        $finish(errors);
      end
      @(posedge clk);
    end while (1'b1);
